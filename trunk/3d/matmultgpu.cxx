...
extern void gpumatmult(float*, float*, float *, float*, float*, float*, float*, float*, int*, int, float, int, int);
...
PetscErrorCode mymatmult(Mat A,Vec x,Vec y)
{
  ...
  float sigma;
  float *targetX,*targetY,*targetZ,*targetW,*sourceX,*sourceY,*sourceZ,*sourceG;
  PetscScalar *ax,*ay;
  PetscErrorCode ierr;
  BOTH *both;
  ...
  targetZ = new float [cluster->n*threadsPerBlock];
  ...
  sourceZ = new float [cluster->n*cluster->maxtrunc];
  ...
  for (ic=cluster->icsta; ic<cluster->icend; ic++) {
    ...
    for (i=0; i<isize; i++) {
      ...
      targetZ[im] = float(particle->zil[i+ista]);
    }
    ...
    for (j=0; j<cluster->nptruncj; j++) {
      ...
      sourceZ[jc] = float(cluster->zjt[j]);
  ...
  gpumatmult(targetX,targetY,targetZ,targetW,sourceX,sourceY,sourceZ,sourceG,
             offset,iblok,sigma,cluster->n,cluster->maxtrunc);
